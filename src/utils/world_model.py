# official code from https://github.com/nicklashansen/tdmpc2/blob/main/tdmpc2/common/world_model.py
from copy import deepcopy

import torch
import torch.nn as nn

from src.utils import neural_nets
from utils import math, init
...
class WorldModel(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        if cfg.multitask:
            self._task_emb = neural_nets.Embedding(
        ...
        self._encoder = neural_nets.enc(cfg)
        self._dynamics = neural_nets.mlp(
            cfg.latent_dim + cfg.action_dim + cfg.task_dim,
            2 * [cfg.mlp_dim],
            cfg.latent_dim,
            act=neural_nets.SimNorm(cfg),
        )
        self._reward = neural_nets.mlp(
            cfg.latent_dim + cfg.action_dim + cfg.task_dim,
            2 * [cfg.mlp_dim],
            max(cfg.num_bins, 1),
        )
        self._termination = (
            neural_nets.mlp(cfg.latent_dim + cfg.task_dim, 2 * [cfg.mlp_dim], 1)
            if cfg.episodic
            else None
        )
        self._pi = neural_nets.mlp(
            cfg.latent_dim + cfg.task_dim, 2 * [cfg.mlp_dim], 2 * cfg.action_dim
        )
        self._Qs = neural_nets.Ensemble(
            [
                neural_nets.mlp(
                    cfg.latent_dim + cfg.action_dim + cfg.task_dim,
                    2 * [cfg.mlp_dim],
